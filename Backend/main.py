...
c.execute('CREATE TABLE IF NOT EXISTS Directivos (Directivos_id INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT NOT NULL, apellido TEXT NOT NULL, mail TEXT, numero_telefono INTEGER, cargo TEXT, id_escuela INTEGER, FOREIGN KEY (id_escuela) REFERENCES Escuelas(id))')
c.execute('CREATE TABLE IF NOT EXISTS Notas (Notas_id INTEGER PRIMARY KEY AUTOINCREMENT, solicitud TEXT, fecha TEXT, direccion_url TEXT, categoria TEXT, id_escuela INTEGER, FOREIGN KEY (id_escuela) REFERENCES Escuelas(id))')
conexion.commit()
conexion.close()
'''

db : Database_Manager = Database_Manager ("Backend/consejo_escolar")
Escuelas : type = db.get_table_class ("Escuelas")
Directivos : type = db.get_table_class ("Directivos")
Notas : type = db.get_table_class ("Notas")

def opciones(campos : tuple):
	'''
	El primer elemento de la tupla (campos[0]) debe de poseer la opcion a realizar el CRUD en str
	El segundo elemento de la tupla (campos[1]) debe de poseer el nombre de la tabla en str
	Al ACTUALIZAR o Eliminar usar el tercer elemento de la tupla (campos[2]) para poseer el id de la tabla seleccionada en str
	Al ACTUALIZAR, a partir del cuarto elemento de la tupla (campos[3]), ingresar valores para cambiar la fila de la tabla (incluyendo valores que no se cambiaran)
	Al BUSCAR, con el tercer elemento de la tupla (campos[2]), establecer el campo de la base de datos en la que se filtra
	Al BUSCAR, el cuarto elemento de la tupla (campos[3]), debe poseer los valores de la barra de busqueda
	'''
	match campos[0]:
		case "insertar":
			match campos[1]:
				case "Escuelas":
					db.insert (Escuelas (numero_escuela = campos[2], nombre = campos[3], direccion = campos[4], categoria = campos[5]))
				case "Directivos":
					db.insert (Directivos (nombre = campos[2], apellido = campos[3], mail = campos[4], numero_telefono = campos[5], cargo = campos[6], id_escuela = campos[7]))
				case "Notas":
					db.insert (Notas (solicitud = campos[2], fecha = campos[3], direccion_url = campos[4], categoria = campos[5], id_escuela = campos[6]))
				case _:
					print('Tabla no valida.')
		case "actualizar":
			match campos[1]:
				case "Escuelas":
					db.update ("Escuelas", condition=f"Escuelas_id = '{campos[2]}'", numero_escuela = campos[3], nombre = campos[4], direccion = campos[5], categoria = campos[6])
				case "Directivos":
					db.update ("Directivos", condition=f"Directivos_id = '{campos[2]}'", nombre = campos[3], apellido = campos[4], mail = campos[5], numero_telefono = campos[6], cargo = campos[7], id_escuela = campos[8])
				case "Notas":
					db.update ("Notas", condition=f"Notas_id = '{campos[2]}'", solicitud = campos[3], fecha = campos[4], direccion_url = campos[5], categoria = campos[6], id_escuela = campos[7])
				case _:
					print('Tabla no valida.')
		case "eliminar":
			match campos[1]:
				case "Escuelas":
					db.delete("Escuelas", condition=f"Escuelas_id = {campos[2]}")
				case "Directivos":
					db.delete("Directivos", condition=f"Directivos_id = {campos[2]}")
				case "Notas":
					db.delete("Notas", condition=f"Notas_id = {campos[2]}")
				case _:
					print('Tabla no valida.')
		case "buscar":
			match campos[1]:
				case "Escuelas":
					return db.select("Escuelas", condition=f"{campos[2]} LIKE '{campos[3]}%'", ordered="nombre")
				case "Directivos":
					return db.select("Directivos", condition=f"{campos[2]} LIKE '{campos[3]}%'", ordered="nombre")
				case "Notas":
					return db.select("Notas", condition=f"{campos[2]} LIKE '{campos[3]}%'", ordered="fecha")
				case _:
					print('Tabla no valida.')
		case "listar":
			match campos[1]:
				case "Escuelas":
					return db.select("Escuelas",  ordered="nombre")
				case "Directivos":
					return db.select("Directivos", ordered="nombre")
				case "Notas":
					return db.select("Notas", ordered="fecha")
				case _:
					print('Tabla no valida.')
		case _:
...
while(a!=0):
	a = input()
	#print(db.select("Escuelas", condition=f"nombre LIKE 'ff%'"))
	c= ('buscar', 'Escuelas', 'Escuelas_id', a)
	print(opciones(c))
	#c= ('eliminar', 'Escuelas', input())
	#c= ('insertar', 'Escuelas', input(), input(), input(), input())
	#print(opciones(c))
#
#OBJETIVO: Filtrado de datos por interno o externo de carmen de patagones, por id en la barra de busqueda o por nombre de escuela.
#OBJETIVO: Listar las escuelas(antes preguntar en que formato se  mostrarán los datos, tuplas, diccionarios, cadenas, etc) y filtrarlas correctamente.
db.disconnect ()