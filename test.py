...
class MainApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Rutas de los logos
        self.logo_claro = "C:/Users/PC/Desktop/Consejo/log/log_claro.png"
        self.logo_oscuro = "C:/Users/PC/Desktop/Consejo/log/log_oscuro.png"
    ...
    def create_sidebar(self):
        ...
        self.schools_button = ctk.CTkButton(self.sidebar_frame, text="Escuelas", width=180, height=40, corner_radius=10, command=self.show_school_screen)
        self.schools_button.grid(row=2, column=0, padx=20, pady=10)

        self.notes_button = ctk.CTkButton(self.sidebar_frame, text="Notas", width=180, height=40, corner_radius=10, command=self.show_notes_screen)
    ...
    def show_school_screen(self):
        ...
        # Crear la pantalla de escuelas dentro de content_frame
        filter_frame = ctk.CTkFrame(self.content_frame, height=50)
        filter_frame.grid(row=0, column=0, sticky="ew", padx=20, pady=(0, 10))

        filter1 = ctk.CTkCheckBox(filter_frame, text="Carmen de Patagones")
        filter1.pack(side="left", padx=10)

        filter2 = ctk.CTkCheckBox(filter_frame, text="Interior")
        filter2.pack(side="left", padx=10)

        filter3 = ctk.CTkEntry(filter_frame, placeholder_text="Nombre")
        filter3.pack(side="left", padx=10, fill="x", expand=True)

        close_button = ctk.CTkButton(filter_frame, text="X", width=30)
        close_button.pack(side="left", padx=10)

        # Listado de escuelas debajo de los filtros
        schools_list_frame = ctk.CTkFrame(self.content_frame)
        schools_list_frame.grid(row=1, column=0, sticky="nsew", padx=20, pady=(10, 0))

        for i in range(4):
            row_frame = ctk.CTkFrame(schools_list_frame, height=40, corner_radius=10)
            row_frame.pack(fill="x", padx=10, pady=5)

            school_label = ctk.CTkLabel(row_frame, text="Escuela", width=200, anchor="w")
            school_label.pack(side="left", padx=10)

            location_label = ctk.CTkLabel(row_frame, text="Ubicación", anchor="e")
            location_label.pack(side="left", padx=10, fill="x", expand=True)
    ...
    def show_notes_screen(self):
        ...
        # Crear la pantalla de notas dentro de content_frame
        ...
        self.folder_image = ctk.CTkImage(Image.open("C:/Users/PC/Desktop/Consejo/img/Folder.png").resize((50, 50)))
    ...
    def update_logo(self):
        current_mode = ctk.get_appearance_mode()
        if current_mode == "Light":
            logo_image = ctk.CTkImage(Image.open(self.logo_claro), size=(180, 180))
        else:
    ...
    def update_sidebar_color(self):
        ...
        if current_mode == "Dark":
            self.sidebar_frame.configure(fg_color="#414141")
        else:
